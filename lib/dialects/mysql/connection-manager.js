        ...
        if (config.pool.handleDisconnects) {
          ...
          connection.on('error', (err) => {
            if (err.code === 'PROTOCOL_CONNECTION_LOST') {
              ...
              this.pool.destroy(connection).catch((e) => {
                // ISSUE(7625) No-op if generic-pool is unable to release the connection, it may have already been released.
                if (!/Resource not currently part of this pool/.exec(e.message)) {
                  throw e;
                }
              });
            }
